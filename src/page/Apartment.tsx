...
import { apartmentCondominiumQuery } from '../util/url/url';
...
const Apartment = () => {
    ...
    const [state, setState] = React.useState({
        ...
        apartmentCondominiums: undefined as Readonly<any> | undefined,
        hoveredAccommodationID: undefined as number | undefined,
    });

    const { apartmentCondominiums, region, hoveredAccommodationID } = state;

    React.useEffect(() => {
        fetch(
            apartmentCondominiumQuery({
                region: 'BTHO',
                page: undefined,
                rental: undefined,
                address: undefined,
            ...
            .then((json) => {
                ...
                const { apartmentCondominiums, region } = json;
                setState((prev) => ({
                    ...prev,
                    region,
            ...
            <DisplayContainer>
                <AccommodationContainer>
                    ...
                    <NumberOfAccommodationFounded>
                        {(apartmentCondominiums ?? []).length}{' '}
                        apartment/condominiums found in {region}
                    </NumberOfAccommodationFounded>
                    {(apartmentCondominiums ?? []).map((house: any) => (
                        <AccommodationInfo
                            key={house.id}
                            onMouseEnter={() =>
                                setState((prev) => ({
